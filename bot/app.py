...
async def process_approval(payment_id, bot_instance):
    ...
    try:
        ...
        invite_link = f"Contact Admin for Channel Access (Channel ID: {channel_id})"
...
async def bot_config_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        f"👤 *Admin Name:* {config.get('admin_name')}\n"
...
def web_save_demo_video():
    ...
    if 'video_file' not in request.files:
        return redirect(url_for('dashboard', err="No video file selected."))

    file = request.files['video_file']
    if file.filename == '':
        return redirect(url_for('dashboard', err="No selected video file."))

    try:
        clean_filename = f"demo_{int(time.time())}_{file.filename.replace(' ', '_')}"
        save_path = DEMO_UPLOADS_DIR / clean_filename
        file.save(save_path)

        if not title:
            title = f"🎬 Demo Video: {file.filename}"

        rel_path = f"uploads/demo_videos/{clean_filename}"
        video_entry = {
        ...
        return redirect(url_for('dashboard', err=f"Failed to upload video: {e}"))
